...
func TestAllocatorIsUpdated(t *testing.T) {
	...
	t.Run(
		"is updated on cloud with only this resource", func(t *testing.T) {
			// Arrange
	...
	t.Run(
		"is not updated on other resources", func(t *testing.T) {
			// Arrange
			ctx, log, _, ad, phase := setupAllocator(t)
	...
	t.Run(
		"is updated on this and other resources", func(t *testing.T) {
			// Arrange
			ctx, log, _, ad, phase := setupAllocator(t)
			obj1 := createObject("sukhov", "obj1", "default")
			_, err := ad.Create(
			...
			obj3 := createObject("gulchatay", "obj3", "default")
			_, err = ad.Create(
				ctx, obj3.Spec.ServiceAccountID, sakeyconfig.GetStaticAccessKeyDescription(obj3.ClusterName, obj3.Name),
			)
			require.NoError(t, err)

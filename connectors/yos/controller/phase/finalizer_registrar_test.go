...
func TestIsUpdated(t *testing.T) {
	t.Run(
		"empty finalizers means not updated", func(t *testing.T) {
			// Arrange
			resource := connectorsv1.YandexObjectStorage{
		...
		"finalizer exist means updated", func(t *testing.T) {
			...
			resource := connectorsv1.YandexObjectStorage{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "resource",
					Namespace:  "default",
					Finalizers: []string{yosconfig.FinalizerName},
				},
			}
			c := k8sfake.NewFakeClient()
			phase := FinalizerRegistrar{
		...
		"finalizer and others exist means updated", func(t *testing.T) {
			...
			resource := connectorsv1.YandexObjectStorage{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "resource",
					Namespace:  "default",
					Finalizers: []string{"not.that.finalizer", yosconfig.FinalizerName, "yet.another.false.finalizer"},
				},
			}
			c := k8sfake.NewFakeClient()
			phase := FinalizerRegistrar{
		...
		"cleanup on non-empty finalizer list removes finalizer", func(t *testing.T) {
			...
			resource := connectorsv1.YandexObjectStorage{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "resource",
					Namespace:  "default",
					Finalizers: []string{"not.that.finalizer", yosconfig.FinalizerName, "yet.another.finalizer"},
				},
			}
			c := k8sfake.NewFakeClient()
			log := logrfake.NewFakeLogger(t)

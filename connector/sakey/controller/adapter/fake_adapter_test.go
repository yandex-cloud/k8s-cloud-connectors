...
func TestRead(t *testing.T) {
	t.Run(
		"read on one object returns one needed", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey := createSakey("sakey", "default", "abdullah")
			skResp, err := ad.Create(
				ctx, sakey.Spec.ServiceAccountID,
				sakeyconfig.GetStaticAccessKeyDescription(sakey.ClusterName, sakey.Name),
			)
	...
	t.Run(
		"read on multiple objects returns one needed", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey1 := createSakey("sakey1", "default", "abdullah")
			sakey2 := createSakey("sakey2", "default", "sukhov")
			_, err := ad.Create(
				ctx, sakey1.Spec.ServiceAccountID,
				sakeyconfig.GetStaticAccessKeyDescription(sakey1.ClusterName, sakey1.Name),
			)
			require.NoError(t, err)
			skResp, err := ad.Create(
				ctx, sakey2.Spec.ServiceAccountID,
				sakeyconfig.GetStaticAccessKeyDescription(sakey2.ClusterName, sakey2.Name),
			)
			require.NoError(t, err)
	...
	t.Run(
		"read on no object throws", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			...
			// Assert
			assert.Error(t, err)
			assert.True(t, errorhandling.CheckRPCErrorNotFound(err))
		},
	)
}
...
func TestList(t *testing.T) {
	t.Run(
		"list on one object returns list of it", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey := createSakey("sakey", "default", "abdullah")
			skResp, err := ad.Create(
	...
	t.Run(
		"list on multiple objects returns list of them", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey1 := createSakey("sakey1", "default", "abdullah")
			sakey2 := createSakey("sakey2", "default", "sukhov")
			...
			require.NoError(t, err)
			sk2Resp, err := ad.Create(
				ctx, sakey2.Spec.ServiceAccountID,
				sakeyconfig.GetStaticAccessKeyDescription(sakey2.ClusterName, sakey2.Name),
			)
			require.NoError(t, err)
			sk3Resp, err := ad.Create(
				ctx, sakey3.Spec.ServiceAccountID,
				sakeyconfig.GetStaticAccessKeyDescription(sakey3.ClusterName, sakey3.Name),
			)
			require.NoError(t, err)
	...
	t.Run(
		"list on no objects returns empty list", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
...
func TestDelete(t *testing.T) {
	t.Run(
		"delete on this object deletes it", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey := createSakey("sakey", "default", "abdullah")
			skResp, err := ad.Create(
	...
	t.Run(
		"delete on other objects throws", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey1 := createSakey("sakey1", "default", "abdullah")
			sakey2 := createSakey("sakey2", "default", "sukhov")
			sk1Resp, err := ad.Create(
				ctx, sakey1.Spec.ServiceAccountID,
				sakeyconfig.GetStaticAccessKeyDescription(sakey1.ClusterName, sakey1.Name),
			)
			require.NoError(t, err)
	...
	t.Run(
		"delete on this and other objects deletes this", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
			sakey1 := createSakey("sakey1", "default", "abdullah")
			sakey2 := createSakey("sakey2", "default", "sukhov")
	...
	t.Run(
		"delete on no object throws", func(t *testing.T) {
			// Arrange
			ctx, ad := setup()
